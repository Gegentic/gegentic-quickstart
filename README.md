...
Runs the public, educational `gegentic/*` Docker Hub images together with `docker compose` — `gegentic/api`, `gegentic/api-migration`, `gegentic/gateway`, `gegentic/guardian`, `gegentic/portal` — plus the infrastructure they depend on (Postgres, SuperTokens).
